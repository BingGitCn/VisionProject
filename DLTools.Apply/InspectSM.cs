...
namespace DLTools.Apply
{
    public class InspectSM//分割测试
    {
        ...
        public HTuple LabelIDS = new HTuple();
        ...
        /// <param name="ignoreLabels">忽略的标注。默认应设置成int[1]{1}</param>
        /// <returns></returns>
        public ObservableCollection<HObject> action(HTuple hv_DLModelHandle, HImage image, int[] ignoreLabels, bool isHighAccuracy = false)
        {
            ObservableCollection<HObject> ROIRs = new ObservableCollection<HObject>();
            // Stack for temporary objects
            HObject[] OTemp = new HObject[20];
            LabelIDS = new HTuple();
            // Local iconic variables

            HObject ho_Image, ho_ProcessdImages, ho_Rectangle = null;
            HObject ho_ImageReduced = null, ho_ImagePart = null, ho_EmptyRegionRst;
            HObject ho_NormalRegionRst, ho_allRegion, ho_currentRegion = null;
            HObject ho_SegmentationImageResult = null, ho_ClassRegions = null;
            ...
            HTuple hv_DLSample = new HTuple(), hv_Size = new HTuple();
            HTuple hv_RessultClassIDs = new HTuple(), hv_rows = new HTuple();
            HTuple hv_columns = new HTuple(), hv_count = new HTuple();
            HTuple hv_Width = new HTuple(), hv_Height = new HTuple();
            HTuple hv_overRate = new HTuple(), hv_IMax = new HTuple();
            HTuple hv_JMax = new HTuple(), hv_I = new HTuple(), hv_J = new HTuple();
            HTuple hv_row = new HTuple(), hv_column = new HTuple();
            HTuple hv_ResultCount = new HTuple(), hv_ResultClassNames = new HTuple();
            HTuple hv_ResultRows1 = new HTuple(), hv_ResultColumns1 = new HTuple();
            HTuple hv_ResultRows2 = new HTuple(), hv_ResultColumns2 = new HTuple();
            HTuple hv_DLResult = new HTuple(), hv_isFirst = new HTuple();
            HTuple hv_T0 = new HTuple(), hv_ResultKeys = new HTuple();
            ...
            HOperatorSet.GenEmptyObj(out ho_EmptyRegionRst);
            HOperatorSet.GenEmptyObj(out ho_NormalRegionRst);
            ...
            try
            {
                ...
                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                {
                    HOperatorSet.SetDlModelParam(hv_DLModelHandle, "image_dimensions", ((((hv_ImageDimensions.TupleSelect(
                        0))).TupleConcat(hv_ImageDimensions.TupleSelect(1)))).TupleConcat(hv_ImageDimensions.TupleSelect(
                        2)));
                }
                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                {
                    hv_DLPreprocessParam.Dispose();
                    create_dl_preprocess_param("segmentation", hv_ImageDimensions.TupleSelect(0),
                        hv_ImageDimensions.TupleSelect(1), hv_ImageDimensions.TupleSelect(2), -127,
                        128, "false", "full_domain", new HTuple(), new HTuple(), new HTuple(),
                        new HTuple(), out hv_DLPreprocessParam);
                }
                hv_DLSample.Dispose();
                HOperatorSet.CreateDict(out hv_DLSample);
                hv_Size.Dispose();
                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                {
                    hv_Size = hv_ImageDimensions.TupleSelect(
                        0);
                }
                hv_RessultClassIDs.Dispose();
                hv_RessultClassIDs = new HTuple();

                int Classj = 0;
                for (int i = 0; i < hv_ClassIDs.TupleLength(); i++)
                {
                    if (!((IList)ignoreLabels).Contains(hv_ClassIDs[i].I))
                    {
                ...
                //for (int i = 0; i < ignoreLabels.Length; i++)
                //    HOperatorSet.TupleRemove(hv_RessultClassIDs, ignoreLabels[i] - (i + 1), out hv_RessultClassIDs);

                //tuple_remove (RessultClassIDs, 0, RessultClassIDs)
                ...
                HTuple resizeFactor = new HTuple(1.0);
                try
                {
                    using (HDevDisposeHelper dh = new HDevDisposeHelper())
                    {
                        HOperatorSet.GetDlModelParam(hv_DLModelHandle, "image_resizefactor", out resizeFactor);
                    }
                }
                catch { }
                ...
                int IMax = 0, JMax = 0;
                IMax = (int)(hv_Width.D / (hv_Size.D * hv_overRate.D)) + 1;
                JMax = (int)(hv_Height.D / (hv_Size.D * hv_overRate.D)) + 1;
                ...
                for (int i = 0; i < IMax; i++)
                {
                    for (int j = 0; j < JMax; j++)
                    {
                        ...
                        {
                            row = hv_Size * hv_overRate * j;
                            if ((hv_Height - row - hv_Size) < 0)
                                row = hv_Height - hv_Size;
                        }
                        if (i == 0)
                        {
                            column = 0;
                        ...
                        using (HDevDisposeHelper dh = new HDevDisposeHelper())
                        {
                            ...
                            HOperatorSet.GenRectangle1(out ho_Rectangle, row, column, JMax == 1 ? (hv_Height - 1) : (row + hv_Size - 1), IMax == 1 ? (hv_Width - 1) : (column + hv_Size - 1));
                        }
                        ...
                        {
                            HObject ExpTmpOutVar_0;
                            HOperatorSet.ZoomImageSize(ho_ImagePart, out ExpTmpOutVar_0, hv_ImageDimensions.TupleSelect(
                        ...
                        {
                            HObject ExpTmpOutVar_0;
                            HOperatorSet.ConcatObj(ho_ProcessdImages, ho_ImagePart, out ExpTmpOutVar_0
                                );
                            ho_ProcessdImages.Dispose();
                            ho_ProcessdImages = ExpTmpOutVar_0;
                        }
                        if (hv_rows == null)
                            hv_rows = new HTuple();
                        hv_rows[hv_count] = row;
                        if (hv_columns == null)
                            hv_columns = new HTuple();
                        hv_columns[hv_count] = column;
                        using (HDevDisposeHelper dh = new HDevDisposeHelper())
                        {
                            {
                                HTuple
                ...
                hv_ResultCount = 0;
                if (hv_ResultClassNames == null)
                    hv_ResultClassNames = new HTuple();
                hv_ResultClassNames[0] = 0;
                if (hv_ResultRows1 == null)
                    hv_ResultRows1 = new HTuple();
                hv_ResultRows1[0] = 0;
                if (hv_ResultColumns1 == null)
                    hv_ResultColumns1 = new HTuple();
                hv_ResultColumns1[0] = 0;
                if (hv_ResultRows2 == null)
                    hv_ResultRows2 = new HTuple();
                hv_ResultRows2[0] = 0;
                if (hv_ResultColumns2 == null)
                    hv_ResultColumns2 = new HTuple();
                hv_ResultColumns2[0] = 0;

                hv_DLSample.Dispose();
                gen_dl_samples_from_images(ho_ProcessdImages, out hv_DLSample);
                preprocess_dl_samples(hv_DLSample, hv_DLPreprocessParam);
                hv_DLResult.Dispose();
                HOperatorSet.SetDlModelParam(hv_DLModelHandle, "batch_size", 2);
                HOperatorSet.ApplyDlModel(hv_DLModelHandle, hv_DLSample, new HTuple(), out hv_DLResult);
                ho_EmptyRegionRst.Dispose();
                HOperatorSet.GenEmptyRegion(out ho_EmptyRegionRst);
                ho_NormalRegionRst.Dispose();
                HOperatorSet.GenEmptyRegion(out ho_NormalRegionRst);
                    ...
                    )) - 1); hv_T0 = (int)hv_T0 + 1)
                {
                    ...
                    HOperatorSet.GenEmptyObj(out ho_currentRegion);
                    for (hv_I = 0; (int)hv_I <= (int)((new HTuple(hv_DLResult.TupleLength())) - 1); hv_I = (int)hv_I + 1)
                    {
                        if ((int)(new HTuple(((hv_DLResult.TupleSelect(hv_I))).TupleNotEqual(new HTuple()))) != 0)
                        {
                            using (HDevDisposeHelper dh = new HDevDisposeHelper())
                            {
                                hv_ResultKeys.Dispose();
                                HOperatorSet.GetDictParam(hv_DLResult.TupleSelect(hv_I), "keys", new HTuple(),
                                    out hv_ResultKeys);
                            }
                                ...
                                -1))) != 0)
                            {
                                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                                {
                                    ...
                                    HOperatorSet.GetDictObject(out ho_SegmentationImageResult, hv_DLResult.TupleSelect(
                                        hv_I), "segmentation_image");
                                }

                                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                                {
                                    ho_ClassRegions.Dispose();
                                    HOperatorSet.Threshold(ho_SegmentationImageResult, out ho_ClassRegions,
                                        hv_RessultClassIDs.TupleSelect(hv_T0), hv_RessultClassIDs.TupleSelect(
                                        hv_T0));

                                    if (isHighAccuracy)
                                    {
                                        HOperatorSet.Intersection(ho_ClassRegions, accuracyRegion, out ho_ClassRegions);
                                    }
                                }
                                using (HDevDisposeHelper dh = new HDevDisposeHelper())
                                {
                                    ho_RegionMoved.Dispose();
                                    HOperatorSet.MoveRegion(ho_ClassRegions, out ho_RegionMoved, hv_rows.TupleSelect(
                                        hv_I), hv_columns.TupleSelect(hv_I));
                                }

                                {
                                    HObject ExpTmpOutVar_0;
                                    HOperatorSet.Union2(ho_currentRegion, ho_RegionMoved, out ExpTmpOutVar_0
                                        );
                                    ho_currentRegion.Dispose();
                    ...
                    for (int i = 0; i < c; i++)
                    {
                        HObject sobj = new HObject();
                        HOperatorSet.SelectObj(ho_currentRegion, out sobj, i + 1);
                        HOperatorSet.ZoomRegion(sobj, out sobj, 1.0 / resizeFactor, 1.0 / resizeFactor);
                        HTuple area, r1, c1;
                        HOperatorSet.AreaCenter(sobj, out area, out r1, out c1);
                        if (area > 10)
                        {
                            ROIRs.Add(sobj);
                            LabelIDS[ROIRs.Count - 1] = hv_RessultClassIDs.TupleSelect(hv_T0);
                        }
                    }

                    //if ((int)(new HTuple(hv_isFirst.TupleEqual(1))) != 0)
                    //{
                    //    ho_NormalRegionRst.Dispose();
                    //    ho_NormalRegionRst = new HObject(ho_currentRegion);
                    //    hv_isFirst.Dispose();
                    //    hv_isFirst = 0;
                    //}
                    //else
                    ...
                    //    ho_NormalRegionRst=  ho_NormalRegionRst.ConcatObj(ho_currentRegion);
            ...
            catch (HalconException HDevExpDefaultException)
            {
                ...
                hv_ResultRows1.Dispose();
                hv_ResultColumns1.Dispose();
                hv_ResultRows2.Dispose();
                hv_ResultColumns2.Dispose();
                hv_DLResult.Dispose();
                hv_isFirst.Dispose();
                hv_T0.Dispose();
                hv_ResultKeys.Dispose();
        ...
        public HImage action2(HTuple hv_DLModelHandle, HImage image, int[] ignoreLabels, bool isHighAccuracy = false)
        {
            ...
            try
            {
                ...
                catch { resizeFactor = new HTuple(1); }

                HOperatorSet.GetImageSize(ho_Image, out hv_Width, out hv_Height);
                resultImage.GenImageConst("byte", hv_Width, hv_Height);
                    ...
                    )) - 1); hv_T0 = (int)hv_T0 + 1)
                {
                    ...
                    for (int i = 0; i < c; i++)
                    {
                        HObject sobj = new HObject();
                        HOperatorSet.SelectObj(ho_currentRegion, out sobj, i + 1);
                        resultImage = resultImage.PaintRegion(new HRegion(sobj), hv_RessultClassIDs.TupleSelect(hv_T0), "fill");
                    }
        ...
        public void RunHalcon(HTuple Window)
        {
            ...
            //action(string modelPath,string imagePath);
        }
    }
}